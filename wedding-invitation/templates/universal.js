...
window.MockBackend = {
    ...
    formatCommentItems: function (wishes) {
        if (!wishes || wishes.length === 0) return '<p class="text-center" style="padding: 20px; color: var(--text-secondary);">No wishes yet. Be the first!</p>';
        ...
        if (data instanceof FormData) {
            ...
            if (action === 'newComment') {
                ...
                if (comment) {
                    ...
                    window.MockBackend.saveWish(name, comment, attendance);
                    response.message = 'Your wish has been sent!';
                    isMocked = true;
                }
            } else if (action === 'loadComment') {
                const wishes = window.MockBackend.getWishes();
                response.commentItems = window.MockBackend.formatCommentItems(wishes);
                response.nextComment = 0;
                isMocked = true;
            } else if (action === 'moreComment') {
                const wishes = window.MockBackend.getWishes();
                // For simplicity, we just return all for now or empty if already loaded
                // In a real mock we'd slice based on the 'start' param
                response.commentItems = ''; // No more comments for now in simple mock
                response.nextComment = 0;
                isMocked = true;
                ...
                fetch(SHEET_SCRIPT_URL, {
                    ...
                    body: JSON.stringify({
                        action: 'rsvp',
                        name: rsvpData.sender || 'Guest', // Fallback if name field differs
                        attendance: rsvpData.attendance,
                        persons: rsvpData.persons || 1, // Fallback
                        date: new Date().toLocaleString()
                    })
                ...
                response.rsvp_content = `
                    <div class="rsvp-success" style="text-align:center; padding: 40px 20px; background: var(--background-secondary); border-radius: 24px;">
                        <i class="fas fa-check-circle" style="font-size: 3rem; color: var(--button-background-secondary); margin-bottom: 20px;"></i>
                        <h3 style="font-family: var(--heading-family); margin-bottom: 10px;">RSVP Received!</h3>
                        <p style="color: var(--text-secondary);">Thank you for confirming your presence. We look forward to seeing you!</p>
                        <button type="button" class="rsvp-confirm-btn" onclick="location.reload()" style="margin-top: 20px; max-width: 200px;">Back</button>
                    </div>`;
                isMocked = true;
                ...
                response.wedding_gift_message = `
                    <div class="gift-success" style="text-align:center; padding: 40px 20px; background: var(--background-secondary); border-radius: 24px;">
                        <i class="fas fa-heart" style="font-size: 3rem; color: #ff4d4d; margin-bottom: 20px;"></i>
                        <h3 style="font-family: var(--heading-family); margin-bottom: 10px;">Gift Confirmation Received!</h3>
                        <p style="color: var(--text-secondary);">Thank you for your kindness and generosity. It means a lot to us!</p>
                        <button type="button" class="gift-confirm-btn" onclick="location.reload()" style="margin-top: 20px; max-width: 200px; padding: 12px 24px; background: var(--button-background-secondary); color: white; border: none; border-radius: 50px; cursor: pointer;">Back</button>
                    </div>`;
                isMocked = true;
            }
        }
